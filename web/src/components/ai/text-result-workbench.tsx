...
function CandidateCard({ result, item, slotName, text, preview, applied, busy, actionError, onEdit, onTransition, onPreview, onApply }: { result: Result; item?: Item; slotName: string; text: Text; preview: Preview | null; applied?: Application; busy: boolean; actionError: boolean; onEdit: (itemID: string, structured: Structured) => void; onTransition: (itemID: string, action: "approve" | "reject") => void; onPreview: (itemID: string) => void; onApply: (itemID: string) => void }) {
  ...
  return <Card className={result.effective ? "border-primary/50 shadow-[0_12px_35px_-28px_rgba(16,83,73,0.8)]" : ""}>
    ...
    <CardContent className="space-y-4">
      ...
      {result.kind === "title" ? <TitleEditor draft={draft} idPrefix={result.public_id} locked={locked} setDraft={setDraft} text={text} /> : <SEOEditor draft={draft} idPrefix={result.public_id} locked={locked} setDraft={setDraft} text={text} />}
...
function PreviewPanel({ preview, text, onApply, applied, busy }: { preview: Preview; text: Text; onApply: () => void; applied?: Application; busy: boolean }) {
  const before = preview.before as Structured;
  const after = preview.after as Structured;
  const revision = typeof after.revision === "number" ? after.revision : "—";
  return <div className="rounded-lg border border-primary/30 bg-primary/[0.035] p-4"><div className="flex items-center gap-2 font-medium"><Search className="h-4 w-4 text-primary" />{text.after} · {text.revision} {revision}</div><p className="mt-1 text-xs text-muted-foreground">{text.previewHelp}</p><div className="mt-4 grid gap-3 sm:grid-cols-2"><Snapshot label={text.before} value={before} empty={text.noFormalContent} /><Snapshot label={`${text.after} · ${text.revision} ${revision}`} value={after} empty={text.noFormalContent} /></div><div className="mt-4 flex justify-end">{applied ? <p className="flex items-center gap-2 text-sm font-medium text-success" role="status"><CheckCircle2 className="h-4 w-4" />{text.applied} · {text.revision} {applied.content.revision}</p> : <Button className="min-h-11" disabled={busy} onClick={onApply}><Send className="h-4 w-4" />{text.apply}</Button>}</div></div>;
}
...
function fromDraft(kind: Result["kind"], draft: Draft, original: Structured): Structured { const source_fields = asStrings(original.source_fields); return kind === "title" ? { title: draft.title.trim(), keywords: splitList(draft.keywords), source_fields } : { short_description: draft.shortDescription.trim(), selling_points: splitLines(draft.sellingPoints), long_description: draft.longDescription.trim(), search_keywords: splitList(draft.searchKeywords), source_fields }; }
