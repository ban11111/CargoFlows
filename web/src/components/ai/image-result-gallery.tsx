  ...
  return <section aria-labelledby="image-results-title" className="space-y-4">
    ...
    {query.data.data.map((thread) => {
      ...
      <div className="space-y-5 p-4">{[...thread.turns].reverse().map((turn) => <section className="space-y-3" key={turn.public_id}>
        ...
        {turn.safe_error ? <ErrorNotice code={turn.failure_code} message={aiFailureMessage(turn.safe_error, turn.failure_code, zh)} recovery={turn.status === "needs_attention" ? (zh ? "结果可能已产生。请先联系管理员确认，再决定是否重试，避免重复付费。" : "A result may exist. Ask an administrator to verify before retrying to avoid duplicate charges.") : (zh ? "调整反馈后从任意历史版本再次修改。" : "Adjust the feedback and revise from any historical version.")} requestId={turn.provider_request_id} title={zh ? "失败原因" : "Failure reason"} /> : null}
