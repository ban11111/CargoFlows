...
export function ImageResultGallery({ jobID, language }: { jobID: string; language: "zh" | "en" }) {
    ...
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
      {query.data.data.map((result) => <article className="overflow-hidden rounded-lg border border-border bg-card" key={result.public_id}>
        ...
        <div className="space-y-1.5 p-3 text-xs text-muted-foreground">
          <div className="flex items-center justify-between gap-2"><span className="font-medium text-foreground">{zh ? `候选 ${result.candidate_index}` : `Candidate ${result.candidate_index}`}</span><span>{result.width} × {result.height}</span></div>
