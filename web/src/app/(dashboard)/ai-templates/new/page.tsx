...
function NewAITemplateEditor() {
  ...
  const validate = useMutation({
    ...
    onSuccess(response, request) { setValidation(response); setValidatedFingerprint(request.fingerprint); },
  });
  ...
  function validateDraft() {
    ...
    setErrorCodes(nextErrors);
    if (Object.keys(nextErrors).length || !parsed.success) return;
    const payload = mutationPayload(parsed.data.name_zh, parsed.data.name_en, parsed.data.target_platform, platformPrompt, parsed.data.slots);
