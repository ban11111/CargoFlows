...
export default function NewAITemplatePage() {
  ...
  const [slots, setSlots] = useState<AITemplateSlotInput[]>([]);
  ...
  const validate = useMutation({
    mutationFn: () => apiRequest<Validation>(`/ai-content-template-versions/${version?.public_id}/validate`, { method: "POST" }),
  ...
  function submit(event: FormEvent) {
    ...
    if (!parsed.success) parsed.error.issues.forEach((issue) => { nextErrors[issue.path.join(".")] ??= localError(issue.message, zh); });
    if (!platformPrompt.trim()) nextErrors.platform_prompt = text.platformPromptRequired;
    setErrors(nextErrors);
    if (Object.keys(nextErrors).length || !parsed.success) return;
    create.mutate({ name_zh: parsed.data.name_zh, name_en: parsed.data.name_en, target_platform: parsed.data.target_platform, default_locale: "zh-CN", prompt_compiler_version: "v1", platform_prompt: platformPrompt.trim(), slots: parsed.data.slots.map((slot, sequence) => toMutationSlot(slot, sequence)) });
  }

  function addSlot(kind: SlotKind) { setSlots((current) => [...current, defaultSlot(kind, current.length + 1)]); setValidation(null); }
  function updateSlot(index: number, patch: Partial<AITemplateSlotInput>) { setSlots((current) => current.map((slot, slotIndex) => slotIndex === index ? ({ ...slot, ...patch } as AITemplateSlotInput) : slot)); setValidation(null); }
...
function IssueList({ issues, title }: { issues: Validation["issues"]; title: string }) { return <div className="rounded-lg border border-danger/30 bg-danger/5 p-4" role="alert"><p className="font-medium text-danger">{title}</p><ul className="mt-2 space-y-1 text-sm text-danger">{issues.map((issue, index) => <li key={`${issue.path}-${index}`}><code>{issue.path}</code>: {issue.message}</li>)}</ul></div>; }

function defaultSlot(kind: SlotKind, number: number): AITemplateSlotInput {
  const base = { slot_key: `${kind === "seo_description" ? "seo" : kind}_${number}`, name_zh: "", name_en: "", prompt_fragment: "" };
