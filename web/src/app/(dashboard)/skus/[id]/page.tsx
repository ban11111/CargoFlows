...
export default function SkuDetailPage() {
  ...
  return (
    <div className="space-y-6">
	  ...
	  <section className="rounded-xl border border-border bg-card p-5 shadow-[var(--shadow-sm)]"><div className="flex flex-wrap items-center justify-between gap-2"><h2 className="text-sm font-semibold">{language === "zh" ? "成本来源与历史" : "Cost sources and history"}</h2>{sku.costing_warning ? <Badge variant="warning">{language === "zh" ? "零成本开账" : "Zero-cost opening"}</Badge> : <Badge variant="success">{language === "zh" ? "已计价" : "Priced"}</Badge>}</div><div className="mt-4 space-y-2">{(costHistory.data?.data ?? []).map((transaction) => { const line = transaction.lines.find((item) => item.sku.public_id === sku.public_id); return line ? <article className="grid gap-2 rounded-lg border border-border p-3 text-sm sm:grid-cols-[130px_1fr_210px]" key={transaction.public_id}><div><Badge variant={transaction.status === "posted" ? "success" : "warning"}>{transaction.status}</Badge>{line.cost_version > 0 ? <Badge className="ml-1" variant="warning">v{line.cost_version}</Badge> : null}<p className="mt-1 font-mono text-xs text-muted-foreground">{transaction.business_date.slice(0, 10)}</p></div><div><p className="font-medium">{transaction.type} · {line.quantity_delta > 0 ? "+" : ""}{line.quantity_delta}</p><p className="mt-1 text-xs text-muted-foreground">{transaction.note || transaction.public_id}</p></div><div className="tabular-nums sm:text-right"><p>{language === "zh" ? "有效移动成本" : "Effective movement"} S${Number(line.effective_movement_cost_sgd).toFixed(8)}</p><p className="text-xs text-muted-foreground">{language === "zh" ? "有效过账后均价" : "Effective avg after"} S${Number(line.effective_average_cost_after_sgd).toFixed(8)}</p>{line.cost_version > 0 ? <p className="text-xs text-muted-foreground">{language === "zh" ? "原始" : "As posted"} S${Number(line.movement_cost_sgd).toFixed(8)} / S${Number(line.average_cost_after_sgd).toFixed(8)}</p> : null}</div></article> : null; })}{costHistory.isSuccess && (costHistory.data?.data ?? []).length === 0 ? <p className="py-6 text-center text-sm text-muted-foreground">{language === "zh" ? "暂无库存交易" : "No inventory transactions"}</p> : null}</div></section>
    </div>
  );
}
