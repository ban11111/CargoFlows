...
export default function NewSKUPage() {
  ...
  return <div className="mx-auto max-w-4xl space-y-6"><header><Button asChild className="mb-3" variant="ghost"><Link href="/skus"><ArrowLeft className="h-4 w-4" />{zh ? "返回 SKU 列表" : "Back to SKUs"}</Link></Button><div className="flex items-center gap-2 text-primary"><PackagePlus className="h-5 w-5" /><p className="text-[11px] font-bold uppercase tracking-[0.16em]">CargoFlows · Catalog</p></div><h1 className="mt-2 text-3xl font-bold tracking-tight text-navy sm:text-4xl">{zh ? "新建 SKU" : "New SKU"}</h1><p className="mt-2 text-sm text-muted-foreground">{zh ? "建立商品、规格、库存预警和平台内容的基础资料。" : "Create the product, variant, stock-alert, and marketplace-content foundation."}</p></header><section className="rounded-xl border border-border bg-card p-5 shadow-[var(--shadow-sm)]"><SKUForm busy={create.isPending} mode="create" onSubmit={(value) => create.mutate(value)} />{create.isError ? <p className="mt-4 rounded-md border border-danger/30 bg-danger/5 p-3 text-sm text-danger" role="alert">{zh ? "创建失败，请检查 SKU 编码是否重复。" : "Creation failed. Check whether the SKU code already exists."}</p> : null}</section></div>;
}
