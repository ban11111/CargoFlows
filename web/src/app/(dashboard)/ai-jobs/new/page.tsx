...
type ExternalReferenceItem = { public_id: string; purpose: "visual_style" | "usage_effect" | "copy_inspiration"; caption_zh: string; caption_en: string; allowed_guidance_zh: string; allowed_guidance_en: string; source_name: string };
type ExternalReferenceSOP = { public_id: string; versions: Array<{ public_id: string; version_number: number; name_zh: string; name_en: string; status: string; items: ExternalReferenceItem[] }> };
type CanvasDraft = { key: string; slotKeys: string[] };
type OptionEntry = { key: string; slot: Slot; requirements?: Slot[]; canvas?: CanvasDraft };
type OutputLanguageMode = "bilingual" | "en" | "zh-CN";
...
function ExternalReferenceGroups({ values, selected, setSelected, allowVisual, zh }: { values: ExternalReferenceSOP[]; selected: string[]; setSelected: (update: (current: string[]) => string[]) => void; allowVisual: boolean; zh: boolean }) {
  const versions = values.flatMap((sop) => sop.versions.filter((version) => version.status === "published"));
  return <fieldset className="space-y-3 rounded-lg border border-border p-4"><legend className="px-1 text-sm font-semibold">{zh ? "AI 参考 SOP（可选）" : "AI reference SOPs (optional)"}</legend><p className="text-xs leading-5 text-muted-foreground">{zh ? "这些图片只提供风格、使用关系或表达灵感，不能作为目标 SKU 的颜色、结构、兼容性、功能或卖点事实。" : "These images provide style, usage relationships, or expression inspiration only. They never establish target-SKU facts."}</p>{versions.map((version) => { const eligible = version.items.filter((item) => allowVisual || item.purpose === "copy_inspiration"); const allSelected = eligible.length > 0 && eligible.every((item) => selected.includes(item.public_id)); return <section className="rounded-lg border border-border bg-muted/20 p-3" key={version.public_id}><div className="flex flex-wrap items-center justify-between gap-2"><div><h3 className="text-sm font-semibold">{zh ? version.name_zh : version.name_en} · V{version.version_number}</h3><p className="mt-1 text-xs text-muted-foreground">{eligible.length} {zh ? "项可用于当前任务" : "references eligible for this job"}</p></div><Button className="min-h-10" disabled={!eligible.length} onClick={() => setSelected((current) => allSelected ? current.filter((id) => !eligible.some((item) => item.public_id === id)) : [...new Set([...current, ...eligible.map((item) => item.public_id)])])} size="sm" type="button" variant="secondary">{allSelected ? (zh ? "取消全选" : "Clear all") : (zh ? "选择全部" : "Select all")}</Button></div><div className="mt-3 grid gap-2 sm:grid-cols-2">{version.items.map((item) => { const disabled = !allowVisual && item.purpose !== "copy_inspiration"; return <label aria-disabled={disabled} className={`flex min-h-20 gap-3 rounded-lg border p-3 ${disabled ? "cursor-not-allowed opacity-50" : "cursor-pointer"} ${selected.includes(item.public_id) ? "border-primary bg-primary/5" : "border-border bg-card"}`} key={item.public_id}><input checked={selected.includes(item.public_id)} className="mt-0.5 h-5 w-5" disabled={disabled} onChange={() => setSelected((current) => current.includes(item.public_id) ? current.filter((id) => id !== item.public_id) : [...current, item.public_id])} type="checkbox"/><Image alt="" unoptimized className="h-14 w-14 rounded object-cover" height={56} loading="lazy" src={`/api/proxy/ai-reference-items/${item.public_id}/media`} width={56}/><span className="min-w-0 text-sm"><span className="block font-medium">{zh ? item.caption_zh : item.caption_en}</span><span className="mt-1 block text-xs text-muted-foreground">{item.purpose.replaceAll("_", " ")} · {item.source_name}</span></span></label>; })}</div>{!allowVisual && version.items.some((item) => item.purpose !== "copy_inspiration") ? <p className="mt-2 text-xs text-muted-foreground">{zh ? "当前只选择了文案输出，视觉风格和使用效果参考已禁用。" : "Visual-style and usage-effect references are disabled for text-only output."}</p> : null}</section>; })}{versions.length === 0 ? <p className="text-sm text-muted-foreground">{zh ? "当前商品分类暂无已发布的 AI 参考 SOP。" : "No published AI reference SOP exists for this product category."}</p> : null}</fieldset>;
}

function BrandIconGroups({ values, selected, setSelected, brandID, loading, zh }: { values: BrandIcon[]; selected: string[]; setSelected: (value: string[]) => void; brandID: string; loading: boolean; zh: boolean }) {
  return <fieldset className="space-y-3 rounded-lg border border-primary/25 bg-primary/[0.025] p-4"><legend className="px-1 text-sm font-semibold">{zh ? "品牌图标参考（可选）" : "Brand mark references (optional)"}</legend><p className="text-xs leading-5 text-muted-foreground">{zh ? "默认选择全部启用图标。生成时保持图形与文字结构，颜色可以随所选风格和背景调整。" : "All active marks are selected by default. Shape and lettering stay fixed; colors may adapt to the selected style and background."}</p><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{values.map((icon) => <label className={`cursor-pointer overflow-hidden rounded-lg border bg-card ${selected.includes(icon.public_id) ? "border-primary ring-2 ring-primary/10" : "border-border"}`} key={icon.public_id}><span className="grid aspect-[3/2] place-items-center bg-muted/35 p-3"><img alt={icon.name} className="max-h-full max-w-full object-contain" src={authenticatedMediaURL(icon.media_url)}/></span><span className="flex items-start gap-2 border-t border-border p-3"><input checked={selected.includes(icon.public_id)} className="mt-0.5 h-5 w-5 accent-[var(--color-primary)]" onChange={() => setSelected(selected.includes(icon.public_id) ? selected.filter((id) => id !== icon.public_id) : [...selected, icon.public_id])} type="checkbox"/><span className="text-sm font-medium">{icon.name}</span></span></label>)}</div>{brandID && !loading && !values.length ? <p className="text-sm text-muted-foreground">{zh ? "该品牌暂无启用的品牌图标。" : "This brand has no active marks."}</p> : null}{!brandID ? <p className="text-sm text-muted-foreground">{zh ? "该 SKU 尚未关联品牌。" : "This SKU is not linked to a brand."}</p> : null}</fieldset>;
}
...
export default function NewAIJobPage() {
  ...
  const [outputLanguageMode, setOutputLanguageMode] = useState<OutputLanguageMode>("bilingual");
  ...
  const [selectedReferenceItems, setSelectedReferenceItems] = useState<string[]>([]);
  ...
  const referencesQuery = useQuery({ queryKey: ["ai-reference-sops", selectedSKU?.product.category_id], queryFn: () => apiRequest<{ data: ExternalReferenceSOP[] }>(`/ai-reference-sops?category_id=${selectedSKU!.product.category_id}`), enabled: Boolean(selectedSKU?.product.category_id) });
  ...
  const createMutation = useMutation({
    mutationFn: () => {
      ...
      const body: components["schemas"]["CreateAIJobRequest"] = {
        ...
        selected_reference_item_ids: selectedReferenceItems,
        output_locales: outputLanguageMode === "bilingual" ? ["en", "zh-CN"] : [outputLanguageMode],
  ...
  return <div className="mx-auto max-w-5xl space-y-6">
    ...
    {allLoaded ? <Card><CardHeader><CardTitle>{stepTitles[step - 1]}</CardTitle></CardHeader><CardContent className="space-y-6">
      {step === 1 ? <div className="grid gap-5 md:grid-cols-2">
        ...
        <div className="space-y-2"><Label htmlFor="job-output-language">{t("outputLanguageStrategy")}</Label><select className={selectClass} id="job-output-language" onChange={(event) => setOutputLanguageMode(event.target.value as OutputLanguageMode)} value={outputLanguageMode}><option value="bilingual">{t("bilingualEnglishFirst")}</option><option value="en">{t("englishOutput")}</option><option value="zh-CN">{t("chineseSimplified")}</option></select><p className="text-xs leading-5 text-muted-foreground">{t("outputLanguageHelp")}</p></div>
