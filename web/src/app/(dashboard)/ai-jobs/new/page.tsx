...
export default function NewAIJobPage() {
  ...
  const assetBlockages = chosenImageSlots.map((slot) => ({ slot, missing: list<string>((slot.constraints as Record<string, unknown>).required_views).filter((view) => !selectedAssetViews.has(view)) })).filter(({ missing }) => selectedAssets.length === 0 || missing.length > 0);
  ...
  return <div className="mx-auto max-w-5xl space-y-6">
    ...
    {allLoaded ? <Card><CardHeader><CardTitle>{stepTitles[step - 1]}</CardTitle></CardHeader><CardContent className="space-y-6">
      ...
      {step === 5 ? <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_320px]"><div className="space-y-4"><div className="rounded-lg border border-warning/30 bg-warning/5 p-4"><div className="flex items-start gap-3"><ShieldCheck className="mt-0.5 h-5 w-5 text-warning" /><div><p className="font-medium">{t("providerDisclosure")}</p><p className="mt-1 text-sm text-muted-foreground">{t("dryRunNotice")}</p><p className="mt-1 text-sm text-muted-foreground">{t("dryRunExplanation")}</p></div></div></div><section className="rounded-lg border border-border p-4"><h3 className="font-medium">{t("disclosedData")}</h3><ul className="mt-3 space-y-2 text-sm text-muted-foreground"><li className="flex gap-2"><Check className="h-4 w-4 text-primary" />{t("productStructuredData")}</li><li className="flex gap-2"><Check className="h-4 w-4 text-primary" />{t("publishedSOP")}</li><li className="flex gap-2"><Check className="h-4 w-4 text-primary" />{t("selectedApprovedImages")}：{selectedAssets.length}</li>{customCanvases ? <li className="flex gap-2"><Check className="h-4 w-4 text-primary" />{zh ? `${canvases.length} 张自定义画布，将生成 ${canvases.length} 张图片` : `${canvases.length} custom canvases will create ${canvases.length} images`}</li> : null}</ul></section></div><aside className="rounded-lg border border-border bg-muted/30 p-4"><h3 className="font-medium">{selectedSKU?.code}</h3><p className="mt-1 text-sm text-muted-foreground">{selection && (zh ? selection.template.name_zh : selection.template.name_en)} · V{selection?.version.version_number}</p><dl className="mt-5 space-y-3 text-sm"><div className="flex justify-between gap-3"><dt className="text-muted-foreground">{t("selectedOutputs")}</dt><dd className="font-medium">{outputCount}</dd></div><div className="flex justify-between gap-3"><dt className="text-muted-foreground">{t("approvedAssets")}</dt><dd className="font-medium">{selectedAssets.length}</dd></div><div className="flex justify-between gap-3"><dt className="text-muted-foreground">{t("expectedCalls")}</dt><dd className="font-medium">{expectedCalls} {t("calls")}</dd></div></dl></aside></div> : null}
