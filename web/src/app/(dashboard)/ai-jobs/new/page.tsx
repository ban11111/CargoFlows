...
export default function NewAIJobPage() {
  ...
  const selectedAssetViews = useMemo(() => new Set(selectedVisualAssets.map((asset) => asset.sop_view_key)), [selectedVisualAssets]);
  const assetBlockages = useMemo(() => chosenSlots.filter((slot) => slot.kind === "image").map((slot) => ({ slot, missing: list<string>((slot.constraints as Record<string, unknown>).required_views).filter((view) => !selectedAssetViews.has(view)) })).filter(({ missing }) => selectedVisualAssets.length === 0 || missing.length > 0), [chosenSlots, selectedAssetViews, selectedVisualAssets.length]);
