...
type SKU = { id: number; code: string; status: string; product: { name: string } };
type Asset = { id: number; review_status: string; captured_at: string; sop_view_name: { "zh-CN": string; en: string }; photo_session_code: string };
type AssetCategory = { skus: Array<{ id: number; assets: Asset[] }> };
...
export default function NewAIJobPage() {
  ...
  const approvedForSKU = useMemo(() => (assetsQuery.data?.data ?? []).flatMap((category) => category.skus).find((sku) => sku.id === Number(skuID))?.assets.filter((asset) => asset.review_status === "approved") ?? [], [assetsQuery.data, skuID]);
  ...
  const createMutation = useMutation({
    mutationFn: () => {
      ...
      const body: components["schemas"]["CreateAIJobRequest"] = {
        sku_id: Number(skuID), template_version_id: versionID, selected_slot_keys: selectedSlots, selected_asset_ids: selectedAssets, locale,
        ...(preference.trim() ? { user_preference: preference.trim() } : {}),
    ...
    onError: () => setError(t("aiJobCreateError")),
  });
  ...
  function selectSKU(next: string) {
    setSkuID(next);
    const assets = (assetsQuery.data?.data ?? []).flatMap((category) => category.skus).find((sku) => sku.id === Number(next))?.assets ?? [];
    setSelectedAssets(assets.filter((asset) => asset.review_status === "approved").map((asset) => asset.id));
  }
  ...
  function next() {
    setError("");
    if (step === 1 && (!skuID || !versionID)) return setError(t("setupRequired"));
    if (step === 2 && selectedSlots.length === 0) return setError(t("selectSlotRequired"));
  ...
  const selectedSKU = skusQuery.data?.data.find((sku) => sku.id === Number(skuID));
      ...
      {allLoaded ? <Card>
        ...
        <CardContent className="space-y-6">
          {step === 1 ? <div className="grid gap-5 md:grid-cols-2">
            <div className="space-y-2"><Label htmlFor="job-sku">{t("selectSKU")}</Label><select className={selectClass} id="job-sku" onChange={(event) => selectSKU(event.target.value)} value={skuID}><option value="">{t("chooseSKU")}</option>{skusQuery.data.data.filter((sku) => sku.status === "active").map((sku) => <option key={sku.id} value={sku.id}>{sku.code} · {sku.product.name}</option>)}</select></div>
