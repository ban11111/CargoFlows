...
export default function AiJobsPage() {
  ...
  const columns: ColumnDef<AiJob>[] = [
	...
	{ accessorKey: "estimated_amount_usd", header: zh ? "任务估算" : "Estimated", cell: ({ row }) => <div className="tabular-nums"><p>${Number(row.original.estimated_amount_usd ?? 0).toFixed(6)}</p><p className="text-[10px] text-muted-foreground">{zh ? "冻结费率" : "Frozen rate"}</p></div> },
	{ accessorKey: "reconciled_amount_usd", header: zh ? "实际分摊" : "Allocated actual", cell: ({ row }) => <div className="tabular-nums"><p>${Number(row.original.reconciled_amount_usd ?? 0).toFixed(6)}</p><Badge className="mt-1" variant={row.original.reconciliation_status === "reconciled_allocation" ? "success" : row.original.reconciliation_status === "unpriced" ? "warning" : "neutral"}>{row.original.reconciliation_status ?? "pending"}</Badge></div> },
    {
      accessorKey: "status",
      header: t("status"),
