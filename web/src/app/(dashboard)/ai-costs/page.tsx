...
import { AlertTriangle, ChevronDown, CircleDollarSign, LoaderCircle, RefreshCw, Scale } from "lucide-react";
...
type CollectionResponse<T> = { data?: T[] | null };
...
export function rowsOrEmpty<T>(response?: CollectionResponse<T>): T[] {
...
export default function AICostsPage() {
  ...
  const summary = useQuery({ queryKey: ["ai-cost-reconciliation", startDate, endDate], queryFn: () => apiRequest<CollectionResponse<Day>>(`/ai-cost/reconciliation?start_date=${startDate}&end_date=${endDate}`) });
	const analytics = useQuery({ queryKey: ["ai-cost-analytics", startDate, endDate, groupBy], queryFn: () => apiRequest<CollectionResponse<AnalyticsRow>>(`/ai-cost/analytics?start_date=${startDate}&end_date=${endDate}&group_by=${groupBy}`) });
  const rates = useQuery({ queryKey: ["ai-cost-rates"], queryFn: () => apiRequest<CollectionResponse<Rate>>("/ai-cost/rates") });
  ...
  const days = rowsOrEmpty(summary.data);
  const analyticsRows = rowsOrEmpty(analytics.data);
  const rateRows = rowsOrEmpty(rates.data);
  ...
  const error = summary.error ?? analytics.error ?? rates.error ?? sync.error ?? addRate.error;
...
export function LineItemDetails({ buckets }: { buckets?: Bucket[] | null }) {
  if (!Array.isArray(buckets) || buckets.length === 0) return <span className="text-muted-foreground">—</span>;

  return <details className="group min-w-40">
    <summary className="flex min-h-11 w-fit cursor-pointer list-none items-center gap-1.5 rounded-md px-2 font-medium text-primary transition-colors hover:bg-primary/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring [&::-webkit-details-marker]:hidden">
      <span>共 {buckets.length} 项</span>
      <ChevronDown aria-hidden="true" className="h-4 w-4 transition-transform group-open:rotate-180" />
    </summary>
    <div className="space-y-1 border-l border-border pl-3 pt-1">
      {buckets.map((bucket) => <p key={bucket.public_id}><span className="font-medium">{bucket.line_item}</span> <span className="tabular-nums text-muted-foreground">{usd(bucket.actual_amount_usd)}</span></p>)}
    </div>
  </details>;
}
