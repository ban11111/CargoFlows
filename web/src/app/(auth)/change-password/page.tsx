...
export default function ChangePasswordPage() {
  const { language } = useLanguage();
  const zh = language === "zh";
  return <main className="flex min-h-dvh items-center justify-center bg-background px-5 py-10"><section className="w-full max-w-md rounded-2xl border border-border bg-card p-6 shadow-[0_20px_60px_rgba(18,34,53,0.08)] sm:p-9" aria-labelledby="change-password-heading"><div className="flex items-center justify-between"><div className="flex items-center gap-3"><span className="flex h-11 w-11 items-center justify-center rounded-xl bg-navy text-white"><Ship className="h-5 w-5" /></span><div><p className="font-bold">CargoFlow</p><p className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground">Secure access</p></div></div><LanguageToggle /></div><p className="mt-10 text-xs font-bold uppercase tracking-[0.16em] text-primary">{zh ? "首次登录" : "First sign-in"}</p><h1 className="mt-2 text-3xl font-bold text-navy" id="change-password-heading">{zh ? "设置你的新密码" : "Set your new password"}</h1><p className="mt-3 text-sm leading-6 text-muted-foreground">{zh ? "管理员提供的是临时密码。完成修改后才能进入 CargoFlow。" : "Your administrator provided a temporary password. Change it before entering CargoFlow."}</p><Suspense fallback={<div className="mt-8 h-72 animate-pulse rounded-xl bg-muted" />}><ChangePasswordForm /></Suspense></section></main>;
}
