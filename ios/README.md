...
If XcodeGen is not installed, create a new iOS App project in Xcode and add the files under `CargoFlows/`.

## Development Settings

- Default API base URL: `https://dev.cargoflows.cc/api/proxy/`
- Seed login after the Go API starts: `admin@cargoflows.cc` / `password123`

The Cloudflare URL is injected through the `CargoFlowsAPIBaseURL` Info.plist value. Change
`CARGOFLOWS_API_BASE_URL` in `project.yml` if the Named Tunnel hostname changes.
...
developer account with `CARGOFLOWS_DEVELOPMENT_TEAM=YOUR_TEAM_ID`.

The signed development IPA is written to `web/public/downloads/CargoFlows.ipa`. It can be
installed with Xcode or Apple Configurator only on devices included in the provisioning profile.

## TestFlight

The project uses Bundle ID `com.cargoflows.app`, marketing version `0.1.0`, and an explicit build
...
3. Register `com.cargoflows.app` and create its app record in App Store Connect.
4. Add a non-transparent 1024x1024 App Store icon under
   `CargoFlows/Assets.xcassets/AppIcon.appiconset`.
...
1. Run `xcodegen generate` and open `CargoFlows.xcodeproj`.
