...
Use a separate `MINIO_AI_BUCKET=cargoflows-ai-private` without public-read policy. Store originals such as `generated/{job_public_id}/{item_public_id}/{turn_public_id}/{candidate_index}-{sha256}.{ext}`. Generate thumbnails server-side only if needed for gallery performance; retain the original immutable object.
